<?php
if ( !class_exists('Wrt_Tuc_Metadata', false) ):
	 ...
	 * @access public
	 */
	abstract class Wrt_Tuc_Metadata {
		...
		protected static function createFromJson($json, $target) {
			...
			if ( empty($apiResponse) || !is_object($apiResponse) || empty($apiResponse->data->update) ){
				$errorMessage = "更新データの解析に失敗しました。";
				do_action('wrt_tuc_api_error', new WP_Error('puc-invalid-json', $errorMessage));
				trigger_error($errorMessage, E_USER_NOTICE);
				return false;
			}

			if ( $apiResponse->success !== true ){
				$errorMessage = "ライセンスキーが間違っています。";
				do_action('wrt_tuc_api_error', new WP_Error('puc-invalid-json', $errorMessage));
				trigger_error($errorMessage, E_USER_NOTICE);
				return false;
			}

			$update = $apiResponse->data->update;

			$valid = $target->validateMetadata($update);
			if ( is_wp_error($valid) ){
				do_action('wrt_tuc_api_error', $valid);
