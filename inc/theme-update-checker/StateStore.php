...
if ( !class_exists('Wrt_Tuc_StateStore', false) ):

	class Wrt_Tuc_StateStore {
		/**
		...
		/**
		 * @var Wrt_Tuc_Update|null Cached update.
		 */
		 ...
		 * @return $this
		 */
		public function setUpdate(Wrt_Tuc_Update $update = null) {
			$this->lazyLoad();
			$this->update = $update;
			return $this;
		}
		...
		public function save() {
			...
			if ( isset($this->update)) {
				...
				if ( Wrt_Tuc_Utils::startsWith($updateClass, $prefix) ) {
		...
		protected function load() {
			...
			$this->lastCheck = intval(Wrt_Tuc_Utils::get($state, 'lastCheck', 0));
			$this->checkedVersion = Wrt_Tuc_Utils::get($state, 'checkedVersion', '');
			$this->update = null;
