...
if ( !class_exists('Wrt_Tuc_UpdateChecker', false) ):

	abstract class Wrt_Tuc_UpdateChecker {
		...
		/**
		 * @var Wrt_Tuc_InstalledPackage
		 */
		protected $package;

		/**
		 * @var Wrt_Tuc_Scheduler
		 */
		public $scheduler;

		/**
		 * @var Wrt_Tuc_UpgraderStatus
		 */
		protected $upgraderStatus;

		/**
		 * @var Wrt_Tuc_StateStore
		 */
		...
		/**
		 * @var Wrt_Tuc_DebugBar_Extension|null
		 */
		...
		public function removeHooks() {
			...
			remove_filter('pre_set_site_transient_' . $this->updateTransient, array($this,'injectUpdate'));
		 ...
		 *
		 * @return Wrt_Tuc_InstalledPackage
		 */
		...
		public function checkForUpdates() {
			...
			add_action('wrt_tuc_api_error', array($this, 'collectApiErrors'), 10, 4);
			...
			remove_action('wrt_tuc_api_error', array($this, 'collectApiErrors'), 10);
		 ...
		 *
		 * @return Wrt_Tuc_Update An instance of Update, or NULL when no updates are available.
		 */
		 ...
		 *
		 * @param Wrt_Tuc_Metadata|null $update
		 */
		protected function fixSupportedWordpressVersion(Wrt_Tuc_Metadata $update = null) {
		 ...
		 *
		 * This method adds the "wrt_tuc_" prefix and the "-$slug" suffix to the filter name.
		 * For example, "pre_inject_update" becomes "wrt_tuc_pre_inject_update-plugin-slug".
		 *
		 * @param string $baseTag
		 * @return string
		 */
		public function getUniqueName($baseTag) {
			$name = 'wrt_tuc_' . $baseTag;
			if ( $this->filterSuffix !== '' ) {
		 ...
		 *
		 * Identical to add_filter(), except it automatically adds the "wrt_tuc_" prefix
		 * and the "-$slug" suffix to the filter name. For example, "request_info_result"
		 * becomes "wrt_tuc_request_info_result-your_plugin_slug".
		 *
		 * @param string $tag
		 ...
		 * @return array [Wrt_Tuc_Metadata|null, array|WP_Error] A metadata instance and the value returned by wp_remote_get().
		 */
		protected function requestMetadata($metaClass, $filterRoot, $queryArgs = array()) {
			...
			if ( !is_wp_error($status) ){
				...
				do_action('wrt_tuc_api_error', $status, $result, $url, $this->slug);
				$this->triggerError(
		...
		protected function validateApiResponse($result) {
			...
			if ( $result['response']['code'] !== 200 ) {
				return new WP_Error(
					'wrt_tuc_unexpected_response_code',
					'HTTP response code is ' . $result['response']['code'] . ' (expected: 200)'
				);
			}

			if ( empty($result['body']) ) {
				return new WP_Error('wrt_tuc_empty_response', 'The metadata file appears to be empty.');
			}
		...
		protected function createDebugBarExtension() {
			return new Wrt_Tuc_DebugBar_Extension($this);
		}

		/**
		 * Display additional configuration details in the Debug Bar panel.
		 *
		 * @param Wrt_Tuc_DebugBar_Panel $panel
		 */
